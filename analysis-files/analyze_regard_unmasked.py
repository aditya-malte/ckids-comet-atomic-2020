...
regard_output_file_path = '/nas/home/malte/ckids-comet-atomic-2020/nlg-bias/models/bert_regard_v2_large/conceptnet_t5small_bias_input_predictions.txt'
...
def print_outlier(keyword, value):
    print("Keyword: {} and value: {}\n".format(keyword, round(value, 4)))

def get_negative_outliers(cutoff, keywords, metrics_dict):
    for keyword in keywords:
        if metrics_dict[keyword]['negative_count'] / metrics_dict[keyword]['total_count'] >= cutoff:
            print_outlier(keyword, metrics_dict[keyword]['negative_count'] / metrics_dict[keyword]['total_count'])

def get_positive_outlier(cutoff, keywords, metrics_dict):
    for keyword in keywords:
        if metrics_dict[keyword]['positive_count'] / metrics_dict[keyword]['total_count'] >= cutoff:
            print_outlier(keyword, metrics_dict[keyword]['positive_count'] / metrics_dict[keyword]['total_count'])
...
if __name__ == "__main__":
    ...
    # get_negative_outliers(0.05, country_keywords, country_metrics_dict)
    ...
    # get_positive_outlier(0.20, religion_keywords, religion_metrics_dict)
    # print("---PROFESSION (negative)\n")
    # get_negative_outliers(0.20, profession_keywords, profession_metrics_dict)
    print("---PROFESSION (positive)\n")
    get_positive_outlier(0.25, profession_keywords, profession_metrics_dict)
